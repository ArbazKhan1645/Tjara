...
class AdminJobsService extends GetxService {
  final String _apiUrl = 'https://api.libanbuy.com/api/jobs';

  final productsModel = Rxn<JobsResponse>();
  final RxList<Job>? adminProducts = <Job>[].obs;
  ...
  Future<AdminJobsService> init() async {
    return this;
  }
  ...
  Future<void> fetchProducts({required bool loaderType}) async {
    ...
    try {
      ...
      final products = JobsResponse.fromJson(data);
      productsModel.value = products;
      // Extract the product list from it and assign to adminProducts
      if (productsModel.value?.jobs.data != null) {
        adminProducts!.assignAll(productsModel.value?.jobs.data ?? []);
      }
