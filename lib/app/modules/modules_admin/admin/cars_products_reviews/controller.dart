...
import 'package:tjara/app/modules/modules_admin/admin/products_review/service.dart';

enum ReviewLoadingState { initial, loading, loaded, error, loadingMore }

class CarsProductReviewController extends GetxController {
  ...
  void _updateReviewData(Reviews reviewsData) {
    if (_currentPage.value == 1) {
      _reviews.value =
          reviewsData.data
              .where((ele) => ele.product?.product.productGroup == "cars")
              .toList();
    } else {
      _reviews.addAll(
        reviewsData.data
